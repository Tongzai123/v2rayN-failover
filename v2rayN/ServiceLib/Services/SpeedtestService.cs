...
public class SpeedtestService(Config config, Func<SpeedTestResult, Task> updateFunc)
{
    ...
    private async Task RunRealPingBatchAsync(List<ServerTestItem> lstSelected, string exitLoopKey)
    {
        var probe = new RealPingProbeService(_config);
        var results = await probe.ProbeAsync(
            ...
            async result =>
            {
                ...
                {
                    await UpdateFunc(result.ProfileId, ResUI.SpeedtestingSkip);
                    return;
                }

                var delay = result.IsSuccess ? result.Delay : -1;
                ProfileExManager.Instance.SetTestDelay(result.ProfileId, delay);
                await UpdateFunc(result.ProfileId, delay.ToString());
            });

        if (!ShouldStopTest(exitLoopKey))
        {
            await FailoverHealthService.Instance.ApplyManualRealPingResultsAsync(results);
        }
    }
