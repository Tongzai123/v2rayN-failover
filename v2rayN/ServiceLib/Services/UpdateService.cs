...
public class UpdateService(Config config, Func<bool, string, Task> updateFunc)
{
    ...
    {
        if (LocalUpdatePolicy.ShouldDisableAppUpdate)
        {
            await UpdateFunc(false, ResUI.menuCheckUpdate + " (Not Support)");
            return;
        }
    ...
    {
        if (LocalUpdatePolicy.IsCoreUpdateDisabled(type))
        {
            await UpdateFunc(false, ResUI.menuCheckUpdate + " (Not Support)");
            return;
        }
    ...
    {
        if (LocalUpdatePolicy.ShouldDisableGeoUpdate)
        {
            await UpdateFunc(false, ResUI.menuCheckUpdate + " (Not Support)");
            return;
        }
