...
public partial class CoreConfigV2rayService
{
    ...
    private void GenBalancer(EMultipleLoad multipleLoad, string selector = Global.ProxyTag)
    {
        ...
        {
            var fallbackTags = _coreConfig.outbounds
                .Select(outbound => outbound.tag)
                .Where(tag => tag == selector || tag.StartsWith($"{selector}-"))
                .Distinct()
                .ToList();

            if (fallbackTags.Count > 0)
            {
                balancer.fallbackTag = fallbackTags.First();
                balancer.strategy.settings.costs = fallbackTags
                    .Select((tag, index) => new BalancersStrategySettingsCosts4Ray
                    {
                        regexp = false,
                        match = tag,
                        value = 1 + index * 1_000_000,
                    })
                    .ToList();
            }
        }
