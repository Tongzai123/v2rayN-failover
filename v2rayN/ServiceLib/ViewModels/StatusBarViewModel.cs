...
public class StatusBarViewModel : MyReactiveObject
{
    ...
    public StatusBarViewModel(Func<EViewAction, object?, Task<bool>>? updateView)
    {
        ...
        AppEvents.TunModeChangeRequested
            .AsObservable()
            .ObserveOn(RxSchedulers.MainThreadScheduler)
            .Subscribe(async result => await SetTunMode(result));
    ...
    {
        if (_config.TunModeItem.EnableTun && TunModeBehavior.ShouldDisableTunForSystemProxy(type))
        {
            _config.TunModeItem.EnableTun = EnableTun = false;
    ...
    private async Task DoEnableTun(bool c)
    {
        ...
        if (EnableTun)
        {
            var normalizedProxyType = TunModeBehavior.NormalizeProxyForTun(_config.SystemProxyItem.SysProxyType, EnableTun);
            if (normalizedProxyType != _config.SystemProxyItem.SysProxyType)
            {
                _config.SystemProxyItem.SysProxyType = normalizedProxyType;
                await ChangeSystemProxyAsync(normalizedProxyType, true);
                SystemProxySelected = (int)_config.SystemProxyItem.SysProxyType;
            }
            else
            {
    ...
    private Task SetTunMode(bool enableTun)
    {
        if (EnableTun == enableTun)
        {
