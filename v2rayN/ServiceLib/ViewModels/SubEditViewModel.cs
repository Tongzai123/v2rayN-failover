...
public class SubEditViewModel : MyReactiveObject
{
    ...
    private async Task SaveSubAsync()
    {
        ...
        var oldItem = SelectedSource.Id.IsNotEmpty()
            ? await AppManager.Instance.GetSubItem(SelectedSource.Id)
            : null;
        var disabledActiveFailoverGroup = oldItem is { IsFailoverGroup: true }
            && !SelectedSource.IsFailoverGroup
            && _config.ActiveFailoverGroupId == oldItem.Id;

        if (await ConfigHandler.AddSubItem(_config, SelectedSource) == 0)
        {
            if (disabledActiveFailoverGroup)
            {
