...
public static class ConfigHandler
{
    ...
    public static Config? LoadConfig()
    {
        ...
        config.GuiItem ??= new();
        config.GuiItem.FailoverRelayFirstByteTimeoutMs = FailoverRelayOptions.NormalizeFirstByteTimeoutMs(config.GuiItem.FailoverRelayFirstByteTimeoutMs);
    ...
    public static void NormalizeFailoverMode(Config config)
    {
        ...
        config.FailoverEnabled = config.FailoverMode == EFailoverMode.Failover;
    }
    ...
    public static async Task<int> SaveProfileDisplayOrder(IReadOnlyList<string> orderedIndexIds)
    {
        var orderedIds = orderedIndexIds
            .Where(id => id.IsNotEmpty())
            .Distinct()
            .ToList();
        ...
        {
            ProfileExManager.Instance.SetSort(orderedIds[i], (i + 1) * 10);
        }
    ...
    {
        if (!TryGetServerHeaderSortColumn(colName, out _))
        {
            return -1;
        }
        ...
        bool asc)
    {
        ...
        IEnumerable<ProfileItemModel> ordered = asc
            ? name switch
            {
    ...
    public static async Task<int> AddSubItem(Config config, SubItem subItem)
    {
        ...
        var disabledFailoverGroup = item is { IsFailoverGroup: true } && !subItem.IsFailoverGroup;
        if (item is null)
        {
            item = subItem;
        }
        else
        {
            ...
            item.IsFailoverGroup = subItem.IsFailoverGroup;
        ...
        {
            if (disabledFailoverGroup)
            {
                if (await FailoverGroupManager.ConvertToNormalGroup(item.Id) != 0)
                {
                    return -1;
                }

                if (config.ActiveFailoverGroupId == item.Id)
                {
                    config.ActiveFailoverGroupId = string.Empty;
                    config.FailoverMode = EFailoverMode.Off;
                    config.FailoverEnabled = false;
                    config.FailoverStartupProfileId = null;
                    await SaveConfig(config);
                }
            }
    ...
    public static List<SubItem> ReorderSubItemsForOrder(IEnumerable<SubItem>? sourceItems, IEnumerable<string?>? orderedIds)
    {
        var items = (sourceItems ?? [])
            .Where(item => item.Id.IsNotEmpty())
            .OrderBy(item => item.Sort)
        ...
        {
            if (id.IsNullOrEmpty() || !itemMap.TryGetValue(id, out var item) || !usedIds.Add(id))
            {
                continue;
            }
        ...
        result.AddRange(items.Where(item => usedIds.Add(item.Id)));
        NormalizeSubItemSort(result);
        return result;
    }

    public static async Task<int> SaveSubItemOrder(IEnumerable<string?>? orderedIds)
    {
        var currentItems = await AppManager.Instance.SubItems();
        var orderedItems = ReorderSubItemsForOrder(currentItems, orderedIds);
    ...
    public static async Task<int> DeleteSubItem(Config config, string id)
    {
        ...
        await SQLiteHelper.Instance.DeleteAsync(item);
        await FailoverGroupManager.ClearGroup(item.Id);
