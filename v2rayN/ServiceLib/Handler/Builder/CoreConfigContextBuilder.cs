...
public class CoreConfigContextBuilder
{
    ...
    /// </summary>
    public static async Task<CoreConfigContextBuilderResult> Build(Config config, ProfileItem node, bool suppressFailover = false)
    {
        ...
        if (config.FailoverMode != EFailoverMode.Off && !suppressFailover)
        {
            ...
            if (relayRuntime is not null)
            {
                context = context with
                {
                    ...
                    FailoverRuntimeQueueSignature = FailoverGroupManager.BuildRuntimeQueueSignature(
                        relayRuntime.Candidates
                            .Select(candidate => context.AllProxiesMap[candidate.ProfileId])),
                };
            ...
            if (resolveResult.EffectiveNode != null)
            {
                ...
                context = await RebuildContextForNodeAsync(context, node);
            }
        }
    ...
    private static async Task<CoreConfigContext> RebuildContextForNodeAsync(CoreConfigContext context, ProfileItem node)
    {
        var runCoreType = AppManager.Instance.GetCoreType(node, node.ConfigType);
        var coreType = runCoreType == ECoreType.sing_box ? ECoreType.sing_box : ECoreType.Xray;
        return context with
