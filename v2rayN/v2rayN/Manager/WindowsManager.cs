...
public sealed class WindowsManager
{
    ...
    public async Task<Icon> GetNotifyIcon(Config config)
    {
        try
        {
            var index = TunModeBehavior.GetNotifyIconIndex(config.SystemProxyItem.SysProxyType, config.TunModeItem.EnableTun);
    ...
    {
        var index = TunModeBehavior.GetNotifyIconIndex(config.SystemProxyItem.SysProxyType, config.TunModeItem.EnableTun) + 1;
